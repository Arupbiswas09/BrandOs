  ...
  return (
    <form action={action} className="flex flex-col gap-4 rounded-[14px] border border-[#E1E7E4] bg-white p-6">
      ...
      <label className="block">
        ...
        <input name="email" type="email" autoComplete="email" required defaultValue={state?.email ?? email} className="field text-[13.5px]" />
      </label>
      <label className="block">
        <span className="label">Choose a password</span>
        <input name="password" type="password" autoComplete="new-password" minLength={10} required className="field text-[13.5px]" autoFocus />
        <span className="mt-1.5 block text-[11.5px] text-[#84918B]">At least ten characters.</span>
      </label>
