    ...
    <main className="min-h-screen" style={{ background: hexA(b.primary, 0.04) }}>
      ...
      <div className="px-5 pb-20 sm:px-8"><div className="mx-auto flex max-w-[860px] flex-col gap-4">
        {assets.map((a) => {
          ...
          return (
            <article key={a.id} className="rounded-2xl border border-[#E2E8F0] bg-white p-6">
              <div className="mb-1 font-mono text-[12.5px] font-bold uppercase tracking-[0.12em] text-[#526077]">{a.type}</div>
              <h2 className="m-0 text-[20px] font-semibold tracking-[-0.015em]">{a.name}</h2>
              {a.short && <p className="m-0 mt-1 text-[15px] text-[#475569]">{a.short}</p>}
              {a.copy && (a.copy.headline || a.copy.body) && (
                <div className="mt-5 rounded-xl bg-[#F8FAFC] p-5">
                  ...
                  {a.copy.body && <div className="whitespace-pre-wrap text-[15px] leading-[1.65] text-[#334155]">{a.copy.body}</div>}
                  {(cta || a.copy.cta) && (
              ...
              {(a.url || a.specs) && (
                <dl className="mt-4 grid gap-3 text-[15px] sm:grid-cols-2">
                  {a.url && <div><dt className="text-[13.5px] text-[#526077]">Where it lives</dt><dd className="m-0 break-all"><a className="underline" style={{ color: b.primary }} href={a.url.startsWith("http") ? a.url : `https://${a.url}`} target="_blank" rel="noreferrer">{a.url}</a></dd></div>}
                  {a.specs && <div><dt className="text-[13.5px] text-[#526077]">Specs</dt><dd className="m-0 text-[#334155]">{a.specs}</dd></div>}
                </dl>
              )}
              {files.length > 0 && (
                <div className="mt-4 flex flex-col gap-2 border-t border-[#F1F5F9] pt-4">
                  {files.map((f) => (
                    <a key={f.key} href={`/api/share/${token}/${f.key}`} className="flex items-center gap-3 rounded-lg px-1 py-1 text-[15px] hover:bg-[#F8FAFC]">
                      <span className="min-w-0 flex-1 truncate font-medium">{f.name}</span>
                      <span className="font-mono text-[14px] text-[#526077]">{f.size}</span>
        ...
        {!assets.length && <div className="rounded-2xl border border-dashed border-[#CBD5E1] p-10 text-center text-[15px] text-[#475569]">Nothing has been shared here yet.</div>}
        ...
        {b.colours.length > 0 && (
          <section className="mt-6">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {b.colours.map((c) => (
                <div key={c.hex + c.name} className="overflow-hidden rounded-xl border border-[#E2E8F0] bg-white">
        ...
        <footer className="mt-10 text-center text-[14px] text-[#5B6B80]">Shared privately through BrandOS. Please do not forward this link.</footer>
      </div></div>
    </main>
  );
}
