  ...
  return (
    <div className="fixed inset-0 z-[60]">
      ...
      <div className="absolute inset-y-0 right-0 flex w-[560px] max-w-full animate-slide flex-col items-center justify-center gap-3 bg-white p-10 text-center">
        <div className="text-[15px] font-semibold">This asset is not here any more</div>
        <div className="text-[14px] text-mute-2">It was deleted, or it belongs to a client you cannot see.</div>
    ...
    <div className="fixed inset-0 z-[60]" role="dialog" aria-modal="true" aria-label={a.name}>
      ...
      <div data-scroll className="absolute inset-y-0 right-0 w-[560px] max-w-full animate-slide overflow-y-auto border-l border-line bg-white shadow-[-16px_0_44px_rgba(16,22,20,.10)]">
        <div className="sticky top-0 z-[2] flex flex-wrap items-center gap-2 border-b border-line bg-white/95 px-4 py-3.5 backdrop-blur-[8px] sm:px-6">
          <button type="button" aria-label="Close" onClick={closeAsset} className="px-1.5 py-0.5 text-[15px] text-mute-2 hover:text-ink">←</button>
          ...
          {ws.can("archive") && (
            <button type="button" onClick={() => run(setArchived, "asset", a.id, !a.archived)} className="px-1 py-[5px] text-[12.5px] text-mute-1 hover:text-ink">{a.archived ? "Restore" : "Archive"}</button>
          )}
          {ws.can("del") && (
            <button type="button" onClick={() => open({ kind: "confirm", item: "asset", id: a.id, label: a.name })} className="px-1 py-[5px] text-[12.5px] text-danger hover:underline">Delete</button>
          )}
        </div>

        <div className="px-5 pb-10 pt-[26px] sm:px-7">
          <div className="mb-5 flex items-start gap-4">
            <span className="flex h-14 w-14 flex-none items-center justify-center rounded-[13px] text-[14px] font-bold tracking-[0.03em]" style={{ background: hexA(color, 0.12), color: readable(color) }}>{ws.codeOf(a)}</span>
            <span className="min-w-0 flex-1">
              <Eyebrow className="mb-1.5">{cat === "campaign" ? "Campaign asset" : cat === "master" ? "Master file" : "Global asset"}</Eyebrow>
              <span className="block text-[21px] font-semibold leading-[1.25] tracking-[-0.018em]">{a.name}</span>
              {a.short && <span className="mt-1 block text-[13.5px] text-mute-2">{a.short}</span>}
            </span>
          </div>
          ...
          <div className="mb-5 flex flex-wrap items-center gap-1.5">
            ...
            {canEdit && (Object.keys(ASSET_STATUS) as Asset["status"][]).map((k) => {
              ...
              return (
                <button key={k} type="button" aria-pressed={on} onClick={() => !on && run(setStatus, "asset", a.id, k)} className="rounded-[7px] border px-2.5 py-1 text-[12.5px] font-medium transition"
          ...
          <div className="mb-[22px] flex gap-0.5 overflow-x-auto border-b border-line" role="tablist">
            {tabs.map(([k, label]) => (
              <button key={k} type="button" role="tab" aria-selected={tab === k} onClick={() => setTab(k)}
                className={cx("-mb-px flex-none border-b-2 px-[13px] py-2 text-[13.5px] transition", tab === k ? "border-accent font-semibold text-ink" : "border-transparent font-medium text-mute-2 hover:text-ink")}>
                {label}
              </button>
            ))}
          </div>
...
function Meta({ label, children, border }: { label: string; children: React.ReactNode; border?: "b" | "r" | "br" }) {
  return (
    <div className={cx("px-4 py-[13px]", border?.includes("r") && "border-r border-line", border?.includes("b") && "border-b border-line")}>
      <div className="mb-[3px] text-[11.5px] text-mute-4">{label}</div>
...
export function ReviewBar({ kind, item }: { kind: "asset" | "offer"; item: Pick<Asset, "id" | "review" | "reviewerId" | "ownerId"> }) {
  ...
  return (
    ...
    >
      ...
      {w && (
        <span className="min-w-[110px] flex-1 text-[13.5px] font-semibold text-ink-3">
...
function Overview({ a }: { a: Asset }) {
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex flex-col gap-[7px]">
        {offers.map((o) => (
          <div key={o.id} className="flex items-center gap-2.5 rounded-[10px] border border-line px-[13px] py-[11px]">
            <button type="button" onClick={() => router.push(href.offer(o.id))} className="flex-1 text-left text-[14px] font-medium text-ink hover:text-accent">{o.name}</button>
            <Chip color={ws.segColor(o.segment, o.brandId)} size="xs">{o.segment}</Chip>
            {ws.can("edit") && <button type="button" onClick={() => run(toggleLink, a.id, o.id)} className="flex-none px-1 py-0.5 text-[12px] text-mute-5 hover:text-danger">Unlink</button>}
          </div>
        ))}
        {!offers.length && a.brandId && (
          <div className="rounded-[10px] border border-dashed border-[rgba(201,154,46,.5)] bg-[rgba(201,154,46,.07)] p-4 text-[13.5px] text-warn-ink">
            This asset is not linked to anything, so nobody will find it by walking through the rooms.
          </div>
        )}
        {!offers.length && !a.brandId && <div className="text-[13.5px] text-mute-2">Global Library items are not linked to offers. Clone one into a brand when you run it.</div>}
      </div>
      {src && (
        <div className="mb-[22px] rounded-[10px] bg-wash px-3.5 py-3 text-[13.5px] text-mute-1">
      ...
      {a.url && section("Live URL", <a href={a.url.startsWith("http") ? a.url : `https://${a.url}`} target="_blank" rel="noreferrer" className="break-all text-[14px] text-accent hover:underline">{a.url}</a>)}
      {a.tags.length > 0 && section("Tags", <div className="flex flex-wrap gap-[5px]">{a.tags.map((t) => <span key={t} className="rounded-[5px] bg-chip px-[9px] py-[3px] text-[12px] text-mute-1">{t}</span>)}</div>)}
      {a.ctaId && ws.cta(a.ctaId) && section("Call to action", <CtaButton id={a.ctaId} />)}
      {a.specs && section("Creative specs", <div className="text-[14px] leading-[1.55] text-ink-3">{a.specs}</div>)}
      {a.audienceNotes && section("Audience notes", <div className="text-[14px] leading-[1.55] text-ink-3">{a.audienceNotes}</div>)}
      {a.aiPrompt && section("AI prompt used", <div className="text-[13.5px] italic leading-[1.55] text-mute-1">{a.aiPrompt}</div>)}
      {a.notes && section("Notes", <div className="whitespace-pre-wrap text-[14px] leading-[1.6] text-ink-3 text-pretty">{a.notes}</div>)}
    </div>
  );
}
...
export function CtaButton({ id, size = "md" }: { id: string; size?: "md" | "lg" }) {
  ...
  return (
    <span
      title={c.url}
      className={cx("inline-block rounded-[7px] font-semibold", size === "lg" ? "px-[17px] py-[9px] text-[14px]" : "px-[15px] py-[7px] text-[13.5px]")}
  ...
  return (
    <div>
      <div className="mb-3.5 flex items-center gap-3">
        <span className="text-[14px] font-semibold">{done} of {items.length} done</span>
        ...
        {canTick && <button type="button" onClick={() => run(resetList, a.id)} className="flex-none px-1 py-0.5 text-[12.5px] text-mute-3 hover:text-ink">Reset</button>}
      </div>
      {items.length > 0 && done === items.length && (
        <div className="mb-3 rounded-[11px] border border-[rgba(47,143,98,.28)] bg-[rgba(47,143,98,.09)] px-[15px] py-[11px] text-[13.5px] text-[#1F6F4A]">Every check is done.</div>
      )}
        ...
        {items.map((it, idx) => (
          <div key={idx} className="group flex items-start gap-3 border-t border-divider px-[15px] py-3 first:border-t-0">
            <button type="button" role="checkbox" aria-checked={it.done} disabled={!canTick} onClick={() => run(toggleItem, a.id, idx)}
              className="mt-px flex h-[19px] w-[19px] flex-none items-center justify-center rounded-[5px] text-[12px] font-bold text-white disabled:cursor-default"
              style={{ border: `1.5px solid ${it.done ? "var(--bos-accent)" : "#AFBDB7"}`, background: it.done ? "var(--bos-accent)" : "#FFF" }}>
              {it.done ? "✓" : ""}
            </button>
            <span className="mt-[3px] flex-none font-mono text-[12px] text-mute-5">{String(idx + 1).padStart(2, "0")}</span>
            <span className="flex-1 text-[14.5px] leading-[1.5] text-pretty" style={{ color: it.done ? "#5C6A64" : "#2A3833", textDecoration: it.done ? "line-through" : "none" }}>{it.text}</span>
            {ws.can("edit") && (
              <span className="flex flex-none gap-0.5 opacity-0 transition group-hover:opacity-100 focus-within:opacity-100">
                <button type="button" aria-label="Move up" disabled={idx === 0} onClick={() => run(moveItem, a.id, idx, -1)} className="px-1 text-[12px] text-line-strong hover:text-ink disabled:opacity-30">↑</button>
                <button type="button" aria-label="Move down" disabled={idx === items.length - 1} onClick={() => run(moveItem, a.id, idx, 1)} className="px-1 text-[12px] text-line-strong hover:text-ink disabled:opacity-30">↓</button>
                <button type="button" aria-label="Remove check" onClick={() => run(removeItem, a.id, idx)} className="px-1 text-[13px] text-line-strong hover:text-danger">✕</button>
              </span>
            )}
          </div>
        ))}
        {!items.length && <div className="p-[30px] text-center text-[14px] text-mute-2">No checks yet.</div>}
      </div>
      {ws.can("edit") && (
        <form className="mt-2.5 flex gap-2" onSubmit={async (e) => { e.preventDefault(); const r = await run(addItem, a.id, text); if (r.ok) setText(""); }}>
          <input value={text} onChange={(e) => setText(e.target.value)} placeholder="Add a check" aria-label="Add a check" className="field flex-1 text-[14px]" />
  ...
  return (
    <div>
      <div className="mb-3 flex items-center gap-[11px]">
        <span className="eyebrow text-[10.5px]">Use for</span>
        <span className="text-[14px] font-semibold">{a.promptFor || "—"}</span>
        <span className="flex-1" />
      ...
      <div className="rounded-xl border border-line bg-[#FBFCFC] px-5 py-[18px]">
        <pre className="m-0 whitespace-pre-wrap break-words font-mono text-[13.5px] leading-[1.75] text-ink-2">{a.prompt || "No prompt written yet."}</pre>
      </div>
      <Hint className="mt-3.5">
        {ws.can("edit")
  ...
  if (!has) {
    return (
      <div className="rounded-xl border border-dashed border-line-strong p-9 text-center">
        <div className="mb-[5px] text-[14.5px] font-semibold">No copy written yet</div>
        <div className="mb-4 text-[13.5px] text-mute-2">Headline, body and CTA live here so nobody has to open a design file to read them.</div>
        {ws.can("edit") && (
    ...
    <div>
      <div className="overflow-hidden rounded-xl border border-line">
        <div className="border-b border-line px-[18px] py-4">
          <div className="mb-[5px] flex items-center text-[11.5px] text-mute-4"><span className="flex-1">Headline</span><CopyLink on={copied === "h"} onClick={() => copy(c!.headline, "h")} /></div>
          <div className="text-[16px] font-semibold leading-[1.4] tracking-[-0.01em] text-pretty">{c!.headline}</div>
        </div>
        <div className="border-b border-line px-[18px] py-4">
          <div className="mb-[5px] flex items-center text-[11.5px] text-mute-4"><span className="flex-1">Body</span><CopyLink on={copied === "b"} onClick={() => copy(c!.body, "b")} /></div>
          <div className="whitespace-pre-wrap text-[14.5px] leading-[1.65] text-ink-3 text-pretty">{c!.body}</div>
        </div>
        <div className="flex items-center gap-3 px-[18px] py-4">
          <div><div className="mb-[5px] text-[11.5px] text-mute-4">Call to action</div><div className="text-[14.5px] font-semibold">{c!.cta || "—"}</div></div>
          <span className="flex-1" />
          <Btn size="sm" onClick={() => copy(all, "all")}>{copied === "all" ? "Copied" : "Copy all"}</Btn>
        </div>
      </div>
      <div className="mt-3 flex items-center gap-3 text-[13px] text-mute-3">
...
function CopyLink({ on, onClick }: { on: boolean; onClick: () => void }) {
  return <button type="button" onClick={onClick} className="text-[11.5px] text-mute-4 hover:text-ink">{on ? "Copied" : "Copy"}</button>;
}
        ...
        <div className="rounded-xl border border-line px-[18px] py-1">
          {files.map((f) => (
            <div key={f.name} className="group flex items-center gap-3 border-t border-divider py-3 first:border-t-0">
              <span className="min-w-0 flex-1 truncate text-[14.5px] font-medium">{f.name}</span>
              <span className="flex-none font-mono text-[12.5px] text-[#64716B]">{f.size}</span>
              {f.url ? (
                <a href={f.url} download={f.name} className="flex-none rounded-[7px] border border-line bg-white px-[11px] py-[5px] text-[12.5px] font-semibold text-accent hover:border-accent">Download</a>
              ) : (
                <span title="Listed for reference. The file itself was never uploaded here." className="flex-none rounded-[7px] border border-dashed border-line px-[11px] py-[5px] text-[12.5px] text-mute-4">Not uploaded</span>
              )}
              {canEdit && <button type="button" aria-label={`Remove ${f.name}`} onClick={() => run(removeFile, a.id, f.name)} className="flex-none px-1 text-[13px] text-line-strong opacity-0 transition hover:text-danger group-hover:opacity-100 focus:opacity-100">✕</button>}
            </div>
          ))}
        </div>
      )}
      {!files.length && <div className="rounded-xl border border-dashed border-line-strong p-9 text-center text-[14px] text-mute-2">No files attached yet.</div>}
      {canEdit && (
        <>
          <input ref={input} type="file" multiple hidden onChange={(e) => upload(e.target.files)} />
          <button type="button" disabled={busy} onClick={() => input.current?.click()} className="mt-2.5 w-full rounded-[10px] border border-line bg-white p-2.5 text-[13.5px] font-semibold text-ink-3 hover:border-mute-2 disabled:opacity-60">
        ...
        <div className="mb-6 rounded-xl border border-line px-[18px] py-1">
          {acts.map((x) => (
            <div key={x.id} className="flex items-center gap-[11px] border-t border-divider py-[13px] first:border-t-0">
              ...
              <span className="flex-1 text-[13.5px] text-ink-3">{ws.first(x.userId)} {x.action}{x.field ? ` — ${x.field}` : ""}</span>
              <span className="flex-none text-[12.5px] text-[#64716B]">{ws.ago(x.createdAt)}</span>
            </div>
          ))}
        </div>
      ) : (
        <div className="mb-6 rounded-xl border border-dashed border-line-strong p-9 text-center text-[14px] text-mute-2">No changes recorded since this asset was created.</div>
      )}
      <Eyebrow className="mb-2.5">Earlier versions</Eyebrow>
      {versions === null && <div className="text-[13.5px] text-mute-4">Loading…</div>}
      {versions && !versions.length && <div className="text-[13.5px] text-mute-3">Every edit saves the version before it. Nothing has been edited here yet.</div>}
      {versions && versions.length > 0 && (
        <div className="rounded-xl border border-line px-[18px] py-1">
          {versions.map((v) => (
            <div key={v.id} className="flex items-center gap-3 border-t border-divider py-3 first:border-t-0">
              <span className="flex-none font-mono text-[12.5px] font-semibold text-mute-1">v{v.version}</span>
              <span className="min-w-0 flex-1">
                <span className="block truncate text-[13.5px] text-ink-3">{v.headline || v.name}</span>
                <span className="block text-[12px] text-mute-4">replaced {ws.ago(v.createdAt)}{v.userId ? ` by ${ws.first(v.userId)}` : ""}</span>
              </span>
      ...
      <div className="rounded-xl border border-line p-[18px]">
        <div className="flex items-center gap-3.5">
          <div className="flex-1">
            <div className="mb-[3px] text-[14.5px] font-semibold">Cleared to send</div>
            <div className="text-[13px] leading-[1.5] text-mute-2">Marks this as signed off and safe to put in front of the client.</div>
          </div>
          {ws.can("edit") ? (
            <button type="button" role="switch" aria-checked={a.clientVisible} disabled={pending} onClick={() => run(toggleClientVisible, a.id)}
              className={cx("flex flex-none items-center gap-2 rounded-[7px] border px-[13px] py-1.5 text-[13px] font-medium", a.clientVisible ? "border-ok/40 bg-[rgba(47,143,98,.08)] text-ok" : "border-line bg-white text-ink-3 hover:border-mute-2")}>
              <span className={cx("h-2 w-2 rounded-full", a.clientVisible ? "bg-ok" : "bg-line-strong")} />{label}
            </button>
          ) : (
            <span className="flex-none rounded-[7px] bg-chip px-[13px] py-1.5 text-[13px] font-semibold text-mute-1">{label}</span>
          )}
        </div>
        {a.clientVisible && a.review !== "Approved" && (
          <div className="mt-3 rounded-[9px] bg-[rgba(201,154,46,.08)] px-3 py-2 text-[13px] text-warn-ink">Cleared to send, but nobody has approved it yet.</div>
        )}
      </div>
