...
function DraftPanel({ assetId, onClose }: { assetId: string; onClose: () => void }) {
  ...
  return (
      <Modal onClose={onClose} title="Draft copy with AI" sub={`${a?.name} · written in ${ws.brand(a?.brandId)?.name}'s voice from the offers it supports. Nothing is saved until you pick one.`} width={680}
        footer={<><span className="flex-1 text-[12.5px] text-mute-4">The current copy is kept as an earlier version.</span><Btn onClick={onClose}>Close</Btn><Btn variant="primary" onClick={run} disabled={pending}>{pending ? "Writing…" : options ? "Try again" : "Write three options"}</Btn></>}
        ...
        {error && <div role="alert" className="rounded-[9px] bg-[rgba(194,65,18,.07)] px-3 py-2 text-[13.5px] text-change-ink">{error}</div>}
        {pending && <div className="rounded-xl border border-dashed border-line-strong p-8 text-center text-[14px] text-mute-2">Reading the brief and writing… this takes a few seconds.</div>}
        {!pending && options?.map((o, i) => (
          <div key={i} className="rounded-xl border border-line p-4">
            <div className="mb-1.5 text-[15px] font-semibold leading-[1.4]">{o.headline}</div>
            <div className="mb-2 whitespace-pre-wrap text-[14px] leading-[1.6] text-ink-3">{o.body}</div>
            <div className="mb-3 text-[13.5px] font-semibold">CTA · {o.cta || "—"}</div>
            <div className="flex items-center gap-3 border-t border-divider pt-2.5">
              <span className="flex-1 text-[13px] italic text-mute-3">{o.why}</span>
