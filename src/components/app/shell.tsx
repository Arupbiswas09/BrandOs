      ...
      {BRAND_TABS.map(([key, label]) => {
        ...
        return (
          <Link
            ...
            className={cx("block w-full border-l-2 py-[5px] pr-2.5 text-left text-[12px]", on ? "border-accent bg-soft font-semibold text-ink" : "border-[#E8EDEB] font-medium text-[#7C8A83] hover:text-ink")}
  ...
  const content = (
    <aside className="flex h-full w-[266px] flex-col border-r border-line bg-white theme-fade">
      ...
      <div className="px-3 pb-2.5">
        <button type="button" onClick={() => setCmdk(true)} className="flex w-full items-center gap-2 rounded-[9px] border border-line bg-[#FBFCFC] px-2.5 py-2 text-left text-[12.5px] text-[#71807A] transition hover:border-line-strong hover:bg-white">
          <span className="flex-1">Search everything</span>
          <kbd className="font-mono text-[11px] tracking-[0.04em] text-mute-1">/</kbd>
        </button>
      </div>
      <div className="px-3 pb-2.5">
        <button type="button" onClick={() => setInbox(true)} className="flex w-full items-center gap-[9px] rounded-[9px] border border-line bg-white px-2.5 py-2 text-left text-[12.5px] font-medium text-ink-3 hover:border-mute-2">
          ...
          <span className="flex h-[19px] min-w-[19px] items-center justify-center rounded-[10px] px-1.5 font-mono text-[10.5px] font-bold theme-fade" style={myQueue ? { background: "var(--bos-accent)", color: "var(--bos-on)" } : { background: "#E4EAE7", color: "#7A8781" }}>
            {myQueue}
          </span>
        </button>
      </div>
      <nav data-scroll className="flex-1 overflow-y-auto px-3 pb-3 pt-1" aria-label="Main">
        <Link href="/" className={cx("mb-0.5 flex w-full items-center gap-[9px] rounded-lg px-2.5 py-[7px] text-[13px]", navRow(p.view === "street"))}>
          <span className="w-3.5 text-center text-[11px] opacity-55">◻</span><span>The Street</span>
        </Link>
        <Link href="/library" className={cx("mb-0.5 flex w-full items-center gap-[9px] rounded-lg px-2.5 py-[7px] text-[13px]", navRow(p.view === "library"))}>
          <span className="w-3.5 text-center text-[11px] opacity-55">◫</span><span>Global Library</span>
        </Link>
        <Link href="/team" className={cx("mb-4 flex w-full items-center gap-[9px] rounded-lg px-2.5 py-[7px] text-[13px]", navRow(p.view === "team"))}>
          <span className="w-3.5 text-center text-[11px] opacity-55">◐</span><span className="flex-1">Team</span>
          <span className="font-mono text-[11px] text-mute-4">{ws.d.users.length}</span>
        </Link>
        <div className="eyebrow px-2.5 pb-[7px] tracking-[0.11em]">Clients</div>
        {clients.map(({ c, count, tops }) => (
          <div key={c.id} className="mb-[3px]">
            <Link href={href.client(c.id)} className={cx("flex w-full items-center gap-2 rounded-lg px-2.5 py-1.5 text-[12.5px] font-semibold", p.view === "client" && p.id === c.id ? "bg-soft text-ink" : "text-mute-1 hover:bg-hover")}>
              <span className="flex-1 truncate">{c.name}</span>
              <span className="text-[10.5px] font-medium text-mute-4">{count}</span>
            </Link>
            {tops.map((b) => {
              ...
              return (
                <div key={b.id}>
                  <Link href={href.brand(b.id)} className={cx("flex w-full items-center gap-[9px] rounded-lg py-1.5 pl-5 pr-2.5 text-[12.5px]", navRow(on))}>
                  ...
                  {subs.map((sb) => {
                    ...
                    return (
                      <div key={sb.id}>
                        <Link href={href.brand(sb.id)} className={cx("flex w-full items-center gap-[9px] rounded-lg py-1.5 pl-8 pr-2.5 text-[12.5px]", navRow(son))}>
        ...
        {ws.can("edit") && (
          <button type="button" onClick={() => open({ kind: "client" })} className="mt-1.5 w-full rounded-lg px-2.5 py-[7px] text-left text-[12.5px] text-mute-2 hover:bg-chip hover:text-ink">+ Add client</button>
        )}
      </nav>
      <div className="relative flex items-center gap-[9px] border-t border-line px-3.5 py-[11px] theme-fade">
        ...
        <button type="button" onClick={() => setWhoOpen((v) => !v)} aria-expanded={whoOpen} className="min-w-0 flex-1 text-left">
          <span className="block truncate text-[12px] font-semibold">{me.name} ⌄</span>
          <span className="flex items-center gap-1.5 text-[10.5px] text-mute-4">
            <span>{me.role}</span>
            <span className="rounded-[4px] px-1 font-mono text-[9px] font-bold" style={{ background: hexA(ACCESS_COLOR[me.access], 0.14), color: ACCESS_COLOR[me.access] }}>{me.access}</span>
          </span>
        </button>
        {ws.d.authMode === "demo" && (
          <button type="button" title="Reset demo data" onClick={() => run(resetDemo)} className="p-1 text-[11px] text-mute-5 hover:text-ink">Reset</button>
        )}
  ...
  return (
    <>
      ...
      <div className="absolute bottom-[58px] left-3 right-3 z-50 animate-pop rounded-xl border border-line bg-white p-1.5 shadow-[0_12px_32px_rgba(16,22,20,.14)]">
        {ws.d.authMode === "demo" && (
          <>
            <div className="eyebrow px-2.5 pb-[5px] pt-[7px] text-[9.5px] tracking-[0.11em]">Viewing as</div>
              ...
              {ws.d.users.map((u) => (
                <button key={u.id} type="button" onClick={() => { onClose(); onSwitch(u.id); }} className={cx("flex w-full items-center gap-[9px] rounded-lg px-2.5 py-[7px] text-left hover:bg-chip", u.id === ws.me.id && "bg-soft")}>
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block text-[12px] font-semibold">{u.name}</span>
                    <span className="block text-[10.5px] text-mute-4">{u.role} · {u.access}</span>
                  </span>
                  <span className="flex-none text-[10.5px] text-mute-2">{ws.d.queueCounts[u.id] ?? 0}</span>
                </button>
              ))}
            </div>
        ...
        <div className="px-2.5 pb-1 pt-1.5 text-[9.5px] eyebrow tracking-[0.11em]">Brand colours</div>
        <div className="flex gap-1 px-2 pb-1.5">
          {(["bold", "moderate", "off"] as const).map((m) => (
            <button key={m} type="button" onClick={() => setMode(m)} className={cx("flex-1 rounded-md border px-2 py-1 text-[11px] capitalize", mode === m ? "border-accent bg-soft font-semibold text-ink" : "border-line text-mute-1 hover:border-mute-4")}>{m}</button>
          ))}
        </div>
        <button type="button" onClick={() => { onClose(); open({ kind: "shortcuts" }); }} className="flex w-full items-center rounded-lg px-2.5 py-[7px] text-left text-[12px] text-mute-1 hover:bg-chip">
          <span className="flex-1">Keyboard shortcuts</span><kbd className="font-mono text-[10.5px] text-mute-4">?</kbd>
        </button>
        <form action={signOut}>
          <button type="submit" className="w-full rounded-lg px-2.5 py-[7px] text-left text-[12px] text-mute-1 hover:bg-chip">Sign out</button>
        </form>
      </div>
    </>
  );
        ...
        {crumbs.map((c, i) => {
          ...
          return (
            <span key={i} className={cx("flex items-center gap-[7px]", last ? "min-w-0 flex-none" : "hidden min-w-0 shrink sm:flex")} style={last ? undefined : { flex: `0 ${Math.max(1, c.label.length - 6)} auto` }}>
              {c.href && !last ? (
                <Link href={c.href} className="block min-w-0 truncate text-[13px] font-medium tracking-[-0.005em] text-[#7C8A83] hover:text-ink">{c.label}</Link>
              ) : (
                <span aria-current={last ? "page" : undefined} className={cx("block min-w-0 truncate text-[13px] tracking-[-0.005em]", last ? "font-semibold text-ink" : "font-medium text-[#7C8A83]")}>{c.label}</span>
              )}
              {!last && <span className="flex-none text-[11px] text-line-strong">/</span>}
            </span>
          );
        })}
      </nav>
      {!readOnly && (
        <button type="button" onClick={() => open({ kind: "new" })} className="flex-none rounded-lg bg-accent px-[13px] py-[7px] text-[12.5px] font-semibold text-on-accent transition hover:brightness-110 theme-fade">+ New</button>
      )}
      {readOnly && (
        <span className="hidden flex-none items-center gap-2 rounded-[7px] px-[11px] py-[5px] text-[12px] font-semibold md:flex" style={{ background: hexA(ACCESS_COLOR[me.access], 0.14), color: ACCESS_COLOR[me.access] }}>
