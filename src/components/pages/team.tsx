...
import { Avatar, Btn, Card, Empty, H2, Page, PageHead, Tabs } from "@/components/ui";
  ...
  return (
    <Page>
      <PageHead
        ...
        actions={canAccess && <Btn variant="primary" size="lg" onClick={() => open({ kind: "person" })}><UserPlus aria-hidden className="h-4 w-4" />Invite someone</Btn>}
        ...
        tabs={<Tabs items={TEAM_TABS.filter(([k]) => k !== "limits" || canAccess).map(([k, l]) => ({ key: k, label: l, active: tab === k, onClick: () => setTab(k) }))} />}
      />
      ...
      <Card className="overflow-hidden">
        {ws.d.users.map((u) => {
          ...
          return (
            <div key={u.id} className="flex flex-wrap items-center gap-x-3.5 gap-y-2 border-t border-divider px-5 py-3 transition-colors first:border-t-0 hover:bg-wash">
      ...
      {tab === "groups" && <>
      <H2 className="mb-[5px]" icon={<UsersRound />} right={canAccess && <Btn size="sm" onClick={() => open({ kind: "group" })}><Plus aria-hidden className="h-3.5 w-3.5" />New group</Btn>}>Groups</H2>
      <p className="mb-4 mt-0 max-w-[62ch] text-[15px] text-mute-3">A group is a saved set of clients. Put someone in the pod and they get everything the pod covers — no per-person list to maintain.</p>
      ...
      {!ws.d.groups.length && (
        <Empty art="people" title="No groups yet" body="Make a group for a pod or an account team, then add people to it. They get every client the group covers.">
          {canAccess && <Btn variant="primary" onClick={() => open({ kind: "group" })}>Make a group</Btn>}
        </Empty>
      )}
      </>}
    </Page>
