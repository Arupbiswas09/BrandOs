...
export function Team() {
  ...
  return (
    <Page>
      ...
      />
      <p className="mb-[30px] max-w-[62ch] text-[14.5px] text-[#71807A] text-pretty">Two things decide what a person gets: what they are allowed to do, and which clients they can see. Everything else follows from those.</p>
        ...
        {ACCESS_LEVELS.map((a) => (
          <Card key={a} className="rounded-xl px-[17px] py-[15px]">
            <div className="mb-2 flex items-center gap-2">
              ...
              <span className="font-mono text-[12px] text-mute-3">{ws.d.users.filter((u) => u.access === a).length}</span>
            </div>
            <div className="text-[12px] leading-[1.5] text-mute-1 text-pretty">{ACCESS_NOTE[a]}</div>
          </Card>
        ))}
      </div>
        ...
        {ws.d.users.map((u) => {
          ...
          return (
            <div key={u.id} className="flex flex-wrap items-center gap-x-3.5 gap-y-2 border-t border-divider px-5 py-3.5 first:border-t-0">
              <Avatar initials={u.initials} size={34} mono className="text-[11px] font-bold" />
              <span className="w-[154px] min-w-0 flex-none">
                <span className="flex items-center gap-[7px]"><span className="truncate text-[14px] font-semibold">{u.name}</span>{isMe && <span className="font-mono text-[10px] text-mute-4">You</span>}</span>
                <span className="mt-px block truncate text-[12px] text-mute-3">{u.role}</span>
              </span>
      ...
      <H2 className="mb-[5px]" right={canAccess && <button type="button" onClick={() => open({ kind: "group" })} className="text-[12px] text-mute-2 hover:text-ink">+ New group</button>}>Groups</H2>
      <p className="mb-[13px] mt-0 max-w-[62ch] text-[12.5px] text-mute-3">A group is a saved set of clients. Put someone in the pod and they get everything the pod covers — no per-person list to maintain.</p>
          ...
          return (
            <Card key={g.id} className="rounded-[13px] px-5 py-[18px]">
              <div className="mb-[5px] flex items-baseline gap-[9px]">
                ...
                <span className="flex-1" />
                <span className="font-mono text-[12px] text-mute-3">{members.length} {members.length === 1 ? "person" : "people"}</span>
              </div>
              <div className="mb-3 text-[12.5px] leading-[1.5] text-mute-1">{g.note}</div>
              <div className="mb-3 flex flex-wrap gap-[5px]">
                {g.clientIds.map((c) => ws.client(c)).filter(Boolean).map((c) => <span key={c!.id} className="rounded-[5px] bg-chip px-[9px] py-[3px] text-[11.5px] text-mute-1">{c!.name}</span>)}
              </div>
              <div className="flex items-center gap-[5px] border-t border-divider pt-[11px]">
                {members.map((m) => <Avatar key={m.id} initials={m.initials} size={26} mono title={m.name} className="text-[9.5px] font-bold" />)}
                {!members.length && <span className="text-[12px] text-mute-4">Nobody in it yet</span>}
                <span className="flex-1" />
                {canAccess && (
                  <>
                    <button type="button" onClick={() => open({ kind: "group", draft: g })} className="text-[11.5px] text-mute-3 hover:text-ink">Edit</button>
                    <button type="button" onClick={() => open({ kind: "confirm", item: "group", id: g.id, label: g.name })} className="ml-2 text-[11.5px] text-mute-5 hover:text-danger">Delete</button>
                  </>
                )}
              </div>
            </Card>
...
function AccessChip({ access }: { access: keyof typeof ACCESS_COLOR }) {
  ...
  return <span className="flex-none rounded-[5px] px-[9px] py-[3px] font-mono text-[11px] font-bold tracking-[0.04em]" style={{ background: hexA(c, 0.14), color: c }}>{access}</span>;
}
