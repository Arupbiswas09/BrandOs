      ...
      <Card className="mt-7 flex flex-wrap gap-x-9 gap-y-4 px-6 py-5">
        {stats.map((s) => (
          <div key={s.label}>
            ...
            <div className="mt-0.5 text-[12.5px] text-mute-2">{s.label}</div>
          </div>
        ))}
        <div className="hidden flex-1 sm:block" />
        <div>
          <div className="text-[13px] font-semibold">{ws.user(c.contactId).name}</div>
          <div className="mt-0.5 text-[12.5px] text-mute-2">Primary contact</div>
        </div>
      </Card>

      <div className="mt-10">
        <H2 right={ws.can("edit") && <button type="button" onClick={() => open({ kind: "brand", draft: { clientId: c.id } })} className="text-[12px] text-mute-2 hover:text-ink">+ Add brand</button>}>Buildings on this property</H2>
              ...
              <Card key={b.id} className="overflow-hidden">
                <Link href={href.brand(b.id)} className="flex w-full items-center gap-[15px] p-[22px] text-left" style={{ background: hexA(b.primary, 0.1) }}>
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-0.5 block text-[12.5px] text-mute-1">{b.tagline}</span>
                  </span>
                  <span className="flex-none text-[13px] text-mute-2">→</span>
                </Link>
                <div className="flex gap-3.5 border-t border-line px-[22px] py-3 text-[11.5px] text-[#71807A]">
                ...
                {subs.map((sb) => (
                  <Link key={sb.id} href={href.brand(sb.id)} className="flex w-full items-center gap-2.5 border-t border-divider px-[22px] py-[11px] text-left hover:bg-wash">
                    ...
                    <span className="flex-1 text-[12.5px] font-medium">{sb.name}</span>
                    <span className="text-[10.5px] text-mute-4">Sub-brand</span>
                  </Link>
                ))}
              </Card>
            );
        ...
        {!tops.length && (
          <div className="rounded-[14px] border border-dashed border-line-strong p-10 text-center">
            ...
            <div className="mb-4 text-[12.5px] text-mute-2">A brand is where offers and assets live.</div>
            ...
            {recent.map((a) => {
              ...
              return (
                <button key={a.id} type="button" onClick={() => openAsset(a.id)} className="flex items-start gap-[11px] rounded-xl border border-line bg-white p-3.5 text-left hover:border-mute-2">
                  <span className="flex h-[30px] w-[30px] flex-none items-center justify-center rounded-[7px] text-[8.5px] font-bold" style={{ background: hexA(col, 0.1), color: col }}>{ws.codeOf(a)}</span>
                  <span className="min-w-0 flex-1">
                    <span className="block text-[12.5px] font-semibold leading-[1.35]">{a.name}</span>
                    <span className="mt-[3px] block text-[12px] text-mute-2">{ws.brand(a.brandId)?.name} · {ws.ago(a.updatedAt)}</span>
                  </span>
                </button>
              );
            })}
          </div>
          {!recent.length && <div className="text-[12.5px] text-mute-3">Nothing touched here yet.</div>}
        </div>
        <div>
          <H2>Activity</H2>
          <Card className="px-[18px] py-1">
            {acts.map((a) => (
              <button key={a.id} type="button" onClick={() => (a.type === "asset" ? openAsset(a.itemId) : a.type === "offer" ? router.push(href.offer(a.itemId)) : a.type === "brand" ? router.push(href.brand(a.itemId)) : undefined)}
                ...
                <span className="min-w-0 flex-1 truncate text-[12px] text-ink-3">{ws.first(a.userId)} {a.action} {a.label}{a.field ? ` — ${a.field}` : ""}</span>
                <span className="flex-none text-[11px] text-[#8B9791]">{ws.ago(a.createdAt)}</span>
              </button>
            ))}
            {!acts.length && <div className="py-5 text-center text-[12.5px] text-mute-4">Quiet so far.</div>}
          </Card>
        </div>
      </div>
    </Page>
