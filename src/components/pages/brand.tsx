    ...
    <div className="animate-fade">
      <div className="px-4 pb-12 pt-12 sm:px-8 sm:pt-16" style={{ background: "linear-gradient(180deg,var(--bos-soft) 0%,rgba(255,255,255,0) 100%)" }}>
        <div className="mx-auto max-w-[760px] animate-rise text-center">
          ...
          {parent && <div className="mb-2.5 text-[12px] font-semibold uppercase tracking-[0.08em] text-mute-2">Wing of <Link href={href.brand(parent.id)} className="hover:text-ink">{parent.name}</Link></div>}
          ...
          <div className="mt-[34px] inline-flex max-w-full flex-wrap justify-center overflow-hidden rounded-[14px] border border-line bg-white theme-fade">
            {stats.map((s, i) => (
              <Link key={s.label} href={href.brand(b.id, s.tab)} className={cx("px-5 py-4 text-center hover:bg-wash sm:px-[30px]", i < stats.length - 1 && "border-r border-line")}>
                ...
                <span className="mt-0.5 block text-[13px] text-mute-2">{s.label}</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      <div className="mx-auto max-w-[1060px] px-4 pb-[88px] pt-4 sm:px-8">
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          {actions.map((q) => (
            <button key={q.label} type="button" onClick={q.go} className="flex flex-col rounded-xl border border-line bg-white p-4 text-left transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_6px_16px_rgba(16,22,20,.07)]">
              <span className="mb-1 block text-[14.5px] font-semibold text-accent">{q.label}</span>
              <span className="block text-[13.5px] leading-[1.45] text-mute-2">{q.sub}</span>
            </button>
          ))}
        </div>
        ...
        {(subs.length > 0 || (canEdit && !b.parentId)) && (
          <div className="mt-10">
            <H2 right={canEdit && !b.parentId && <button type="button" onClick={() => open({ kind: "brand", draft: { clientId: b.clientId, parentId: b.id } })} className="text-[13px] text-mute-2 hover:text-ink">+ Add sub-brand</button>}>Wings of this building</H2>
            {subs.length > 0 ? (
              <div className="grid gap-3.5 md:grid-cols-2">
                {subs.map((sb) => (
                  <Link key={sb.id} href={href.brand(sb.id)} className="flex items-center gap-3.5 rounded-[13px] border border-line p-[18px] text-left hover:brightness-[.985]" style={{ background: hexA(sb.primary, 0.1) }}>
                    ...
                    <span className="min-w-0 flex-1"><span className="block text-[14.5px] font-semibold">{sb.name}</span><span className="mt-0.5 block text-[12.5px] text-mute-1">{sb.tagline}</span></span>
                    <span className="flex-none text-[13px] text-mute-2">{plural(live(ws.assetsOf(sb.id)).length, "asset")}</span>
                  </Link>
                ))}
              </div>
            ) : (
              <div className="text-[13.5px] text-mute-3">No sub-brands. Add one when part of this brand needs its own identity.</div>
            )}
          </div>
        )}
        ...
        {camp.length > 0 && (
          <div className="mt-11">
            <div className="mb-1 flex items-baseline justify-between gap-4">
              <h2 className="m-0 text-[14px] font-semibold">Everything in this building</h2>
              <span className="text-[13.5px] text-mute-3">{plural(camp.length, "asset")} in this building</span>
            </div>
            <p className="mb-4 mt-0 text-[13.5px] text-mute-3">One square per asset, coloured by where it stands. Hover any square to name it.</p>
            <Card className="overflow-hidden">
              {byType.map(([type, list]) => (
                <Link key={type} href={`${href.brand(b.id, "assets")}?type=${encodeURIComponent(type)}`} className="flex w-full items-start gap-4 border-t border-divider px-5 py-[13px] text-left first:border-t-0 hover:bg-wash">
                  <span className="w-[110px] flex-none pt-px text-[13.5px] font-semibold sm:w-[136px]">{type}</span>
                  <span className="w-[26px] flex-none pt-0.5 font-mono text-[13px] text-mute-3">{list.length}</span>
            ...
            <div className="mt-3 flex flex-wrap gap-4">
              {[["Approved", rc.Approved, "#2F8F62"], ["In review", rc["In review"], "#8A6A12"], ["Changes requested", rc["Changes requested"], "#C2410C"], ["Not reviewed", rc.None, "#CBD6D1"]].map(([l, n, c]) => (
                <span key={l as string} className="flex items-center gap-[7px] text-[13px] text-mute-2">
        ...
        <div className="mt-11 flex flex-wrap items-center gap-2.5 border-t border-line pt-5">
          ...
          <span className="text-[13px] text-mute-3 md:max-w-[52ch]">Archiving hides a brand and everything in it. Nothing inside changes, and restoring brings it all back as it was.</span>
        </div>
      </div>
    </div>
  );
...
function Services({ b }: { b: Brand }) {
  ...
  return (
    <Page>
      ...
      <p className="mb-[30px] max-w-[60ch] text-[14px] text-[#566560] text-pretty">A service is what you sell. An offer is that service argued at one segment. Keeping them apart is what makes a missing version visible.</p>

      {cols.length > 0 && svcs.length > 0 && (
        <div className="mb-9">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
            <h2 className="m-0 flex-none text-[14px] font-semibold">Coverage</h2>
            ...
            <span className="flex-none text-[13px] font-semibold text-[#8A6A12]">{plural(gaps, "combination")} not written yet</span>
          </div>
          <Card className="overflow-x-auto">
            <div className="min-w-[560px]">
              <div className="flex items-stretch border-b border-line bg-wash-2">
                <div className="eyebrow w-[200px] flex-none px-4 py-3 text-[10.5px] tracking-[0.11em]">Service · {gridBy === "goal" ? "Goal" : gridBy === "type" ? "Offer type" : "Segment"}</div>
                {cols.map((c) => <div key={c} className="flex-1 border-l border-line px-2 py-3 text-center text-[12.5px] font-semibold leading-[1.3] text-mute-1">{c}</div>)}
              </div>
              {rows.map((r) => (
                <div key={r.id} className="flex items-stretch border-b border-divider last:border-b-0">
                  <button type="button" onClick={() => router.push(r.sid ? href.service(r.sid) : href.brand(b.id, "offers"))} className="w-[200px] flex-none px-4 py-[13px] text-left text-[14px] font-medium hover:text-accent">{r.label}</button>
                  {cols.map((col) => {
                    ...
                    return (
                      <button
                        ...
                        className="min-h-[46px] flex-1 border-l border-line text-[14px] font-semibold transition hover:brightness-95 disabled:cursor-default"
          ...
          <p className="mb-0 mt-2.5 text-[13px] text-mute-3">
        ...
        {svcs.map((v) => {
          ...
          return (
            <Link key={v.id} href={href.service(v.id)} className="flex flex-col gap-[13px] rounded-[14px] border border-line bg-white p-[22px] text-left transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_8px_20px_rgba(16,22,20,.07)]">
              <span className="block"><span className="block text-[18px] font-semibold tracking-[-0.015em]">{v.name}</span><span className="mt-[3px] block text-[13.5px] text-mute-2">{v.short}</span></span>
              <span className="block text-[14px] leading-[1.55] text-ink-3 text-pretty">{v.description}</span>
              <span className="flex flex-wrap gap-[5px]">{[...segs].map((n) => <span key={n} className="rounded-[5px] px-2 py-0.5 text-[11.5px] font-semibold" style={{ background: hexA(ws.segColor(n, b.id), 0.13), color: readable(ws.segColor(n, b.id)) }}>{n}</span>)}</span>
              {hasGap && <span className="block text-[12.5px] text-warn-text">Nothing written for {missing.join(", ")}</span>}
              <Blocks blocks={blocksFor(assets)} />
              <span className="flex gap-3.5 border-t border-divider pt-3 text-[13.5px] text-[#566560]"><span>{plural(so.length, "offer")}</span><span>{plural(assets.length, "asset")}</span></span>
            </Link>
          );
        })}
      </div>
      ...
      {stand.length > 0 && (
        <div className="mt-[38px]">
          <h2 className="m-0 mb-1 text-[14px] font-semibold">Standalone offers</h2>
          <p className="mb-3.5 mt-0 text-[13px] text-mute-2">These sit above every service rather than inside one.</p>
...
function Offers({ b }: { b: Brand }) {
  ...
  return (
    <Page>
      ...
      <p className="mb-6 max-w-[56ch] text-[14px] text-[#566560] text-pretty">Each offer holds one piece of positioning and everything that supports it. Assets can sit in several at once.</p>
      <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Filter offers" aria-label="Filter offers" className="field mb-3 max-w-[280px] text-[13.5px]" />
...
function Assets({ b, initialType }: { b: Brand; initialType?: string }) {
  ...
  return (
    <Page>
      ...
      <p className="mb-[22px] text-[14px] text-[#566560]">{plural(liveCamp.length, "asset")} in this building. Each one exists once, however many offers point at it.</p>
      {unlinked.length > 0 && (
      ...
      <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Filter by name or tag" aria-label="Filter by name or tag" className="field mb-3 max-w-[280px] text-[13.5px]" />
...
function Kit({ b }: { b: Brand }) {
  ...
  return (
    <Page>
      ...
      <p className="mb-[34px] max-w-[56ch] text-[14px] text-[#566560] text-pretty">Master files, not campaign work. Nothing in here is tied to an offer.</p>

      <H2 right={canEdit && <button type="button" onClick={() => open({ kind: "asset", draft: { brandId: b.id, type: "Logo", status: "Ready", offerIds: [] }, step: 3 })} className="text-[13px] text-mute-2 hover:text-ink">+ Add master file</button>}>Logos and typefaces</H2>
      <div className="mb-[38px] grid grid-cols-2 gap-3.5 md:grid-cols-3 lg:grid-cols-4">
        ...
        {!logos.length && !fontAssets.length && <div className="col-span-full rounded-[13px] border border-dashed border-line-strong p-6 text-center text-[14px] text-mute-2">No logo files yet.</div>}
      </div>
      ...
      <div className="mb-[38px] grid grid-cols-2 gap-3.5 md:grid-cols-3 lg:grid-cols-4">
        {b.colours.map((c) => (
          <button key={c.name + c.hex} type="button" title="Copy hex" onClick={() => { void navigator.clipboard?.writeText(c.hex).catch(() => {}); toast(`${c.hex} copied`); }} className="flex flex-col overflow-hidden rounded-[13px] border border-line bg-white p-0 text-left hover:border-mute-2">
            <span className="flex h-24 items-end p-3 text-[13px] font-semibold tracking-[0.02em]" style={{ background: c.hex, color: onColor(c.hex) }}>{c.hex}</span>
            <span className="block px-3.5 py-[13px]"><span className="block text-[13.5px] font-semibold">{c.name}</span><span className="mt-[3px] block text-[13px] leading-[1.4] text-mute-2">{c.usage}</span></span>
          </button>
        ))}
        {!b.colours.length && <div className="col-span-full text-[13.5px] text-mute-3">No colours recorded.</div>}
      </div>
          ...
          <Card className="rounded-[13px] px-[18px] py-1.5">
            {b.fonts.map((f) => (
              <div key={f.name + f.role} className="flex items-center gap-3 border-t border-divider py-[13px] first:border-t-0">
                <span className="flex-1"><span className="block text-[14px] font-semibold">{f.name}</span><span className="mt-0.5 block text-[13px] text-mute-2">{f.role}</span></span>
                <span className="text-[12.5px] text-[#64716B]">{f.files}</span>
              </div>
            ))}
            {!b.fonts.length && <div className="py-3 text-[13.5px] text-mute-3">No fonts recorded.</div>}
          </Card>
        </div>
        <div>
          <H2>Guidelines</H2>
          <Card className="rounded-[13px] px-[18px] py-1.5">
            {b.guidelines.map((g) => (
              <div key={g.name} className="flex items-center gap-3 border-t border-divider py-[13px] first:border-t-0">
                <span className="flex-1 truncate text-[14px] font-medium">{g.name}</span>
                <span className="flex-none text-[12.5px] text-[#64716B]">{g.size}</span>
              </div>
            ))}
            {!b.guidelines.length && <div className="py-3 text-[13.5px] text-mute-3">No guideline documents yet. Add a Guidelines master file above.</div>}
          </Card>
        </div>
      </div>
      ...
      {templates.length ? (
        ...
        <div className="mb-[38px] rounded-[13px] border border-dashed border-line-strong p-8 text-center text-[14px] text-mute-2">No templates saved for this brand yet.</div>
      )}

      <H2 className="mb-[5px]" right={canEdit && <button type="button" onClick={() => open({ kind: "goal", brandId: b.id })} className="text-[13px] text-mute-2 hover:text-ink">+ New goal</button>}>Goals</H2>
      <p className="mb-[13px] mt-0 max-w-[64ch] text-[13.5px] text-mute-2">Broad business outcomes this brand is chasing — not channels or tactics. {b.goals.length} goals · five is usually enough. If two of them keep catching the same offers, merge them.</p>
        ...
        {b.goals.map((g) => {
          ...
          return (
            <Card key={g.name} className="rounded-xl px-[18px] py-4">
              ...
              <div className="mb-[11px] min-h-8 text-[13px] leading-[1.5] text-mute-1 text-pretty">{g.description}</div>
              <div className="flex items-center gap-2 border-t border-divider pt-2.5">
                <button type="button" onClick={() => open({ kind: "goalOffers", brandId: b.id, name: g.name })} className="flex-1 text-left text-[12.5px] font-medium hover:underline" style={{ color: n ? "#566560" : "#8A6A12" }}>{n ? plural(n, "offer") : "None yet"}</button>
                {canEdit && <button type="button" onClick={() => open({ kind: "goal", brandId: b.id, name: g.name, description: g.description, original: g.name })} className="text-[12.5px] text-mute-2 hover:text-ink">Edit</button>}
                {canEdit && b.goals.length > 1 && <button type="button" onClick={() => open({ kind: "mergeGoal", brandId: b.id, from: g.name })} className="text-[12.5px] text-mute-2 hover:text-ink">Merge</button>}
              </div>
            </Card>
          );
        })}
      ...
      <h2 className="m-0 mb-[5px] text-[14px] font-semibold">Offer types</h2>
      <p className="mb-[13px] mt-0 max-w-[62ch] text-[13.5px] text-mute-2">What the offer physically is. An audit and a paid engagement can both chase the same goal.</p>
        ...
        {OFFER_TYPES.map((t) => {
          ...
          return (
            <div key={t} className="flex items-center gap-2.5 rounded-[11px] border border-line bg-white px-[15px] py-[11px]">
              ...
              <span className="text-[12.5px] font-medium" style={{ color: n ? "#566560" : "#8A6A12" }}>{n ? plural(n, "offer") : "None yet"}</span>
            </div>
          );
        })}
      </div>

      <H2 right={canEdit && <button type="button" onClick={() => open({ kind: "kit", brandId: b.id })} className="text-[13px] text-mute-2 hover:text-ink">Edit segments</button>}>Segments</H2>
      <div className="mb-[38px] flex flex-wrap gap-2.5">
        {b.segments.map((sg) => (
          <Link key={sg.name} href={`${href.brand(b.id, "offers")}`} className="flex items-center gap-[11px] rounded-[11px] border border-line bg-white px-4 py-3 hover:border-mute-2">
            ...
            <span className="text-[14px] font-semibold">{sg.name}</span>
            <span className="text-[13.5px] text-mute-2">{plural(offers.filter((o) => o.segment === sg.name).length, "offer")}</span>
          </Link>
        ))}
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card className="rounded-[13px] px-[22px] py-5">
          <h2 className="m-0 mb-[9px] text-[14px] font-semibold">Voice and tone</h2>
          <p className="m-0 text-[14px] leading-[1.6] text-ink-3 text-pretty">{b.voice || <span className="text-mute-4">Not written yet.</span>}</p>
        </Card>
        <Card className="rounded-[13px] px-[22px] py-5">
          <h2 className="m-0 mb-[9px] text-[14px] font-semibold">Boilerplate</h2>
...
function TemplateCard({ id, name, short }: { id: string; name: string; short: string }) {
  ...
  return (
    <button type="button" onClick={() => openAsset(id)} className="flex flex-col rounded-[13px] border border-dashed border-line-strong bg-white p-4 text-left hover:border-mute-2">
      <span className="mb-[9px] block font-mono text-[10.5px] font-bold tracking-[0.11em] text-mute-4">TEMPLATE</span>
      <span className="block text-[14px] font-semibold leading-[1.35]">{name}</span>
      <span className="mt-1 block text-[13px] text-mute-2">{short}</span>
    </button>
  );
}
...
function Ctas({ b }: { b: Brand }) {
  ...
  return (
    <Page>
      ...
      <p className="mb-7 max-w-[56ch] text-[14px] text-[#566560] text-pretty">Written once, used everywhere. These render in their real colours so you can see what a reader sees.</p>
      <div className="grid gap-3.5 md:grid-cols-2">
        {ctas.map((c) => {
          const used = [
          ...
          return (
            <Card key={c.id} className="flex flex-col gap-3.5 rounded-[13px] px-[22px] py-5">
              ...
              <div>
                <div className="mb-[3px] break-all text-[13.5px] text-mute-2">{c.url || "No destination set"}</div>
                <div className="text-[12.5px] font-medium" style={{ color: used.length ? "#566560" : "#8A6A12" }}>{used.length ? `Used in ${plural(used.length, "place")}` : "Not used yet"}</div>
                {used.length > 0 && <div className="mt-0.5 text-[12.5px] text-[#64716B]">{used.slice(0, 3).join(" · ")}{used.length > 3 ? ` · +${used.length - 3} more` : ""}</div>}
              </div>
              <div className="flex gap-[7px] border-t border-divider pt-3">
                ...
                <span className="flex-1" />
                <span className="self-center text-[12px] text-[#64716B]">{c.style}</span>
              </div>
            </Card>
          );
        })}
