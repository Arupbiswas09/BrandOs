...
export function Settings() {
  ...
  return (
    <Page className="max-w-[760px]">
      ...
      <p className="mb-8 mt-1 text-[15px] text-mute-1">You are signed in as {me.email ?? me.name} with <strong className="font-semibold text-ink-3">{me.access}</strong> access. {ws.scopeLabel(me) === "Every client" ? "You can see every client." : `You can see: ${ws.scopeLabel(me)}.`}</p>
        ...
        <>
          ...
          <Card className="mb-8 p-6">
            <form action={pwAction} className="flex flex-col gap-4">
              ...
              {pw?.error && <div role="alert" className="text-[13.5px] text-change-ink">{pw.error}</div>}
              {pw?.email === "saved" && <div role="status" className="text-[13.5px] text-ok">Password changed.</div>}
      ...
      <H2>Display</H2>
      <Card className="mb-8 p-6">
        <div className="mb-2 text-[14px] font-semibold">Brand colours</div>
        <p className="mb-3 mt-0 text-[13.5px] text-mute-2">Inside a brand, BrandOS takes on that brand&apos;s colours. Turn it down if it gets in the way.</p>
        <div className="flex flex-wrap gap-2" role="radiogroup" aria-label="Brand colours">
          {([["bold", "Bold — tinted pages"], ["moderate", "Moderate — accents only"], ["off", "Off — always neutral"]] as const).map(([k, label]) => (
            <button key={k} type="button" role="radio" aria-checked={mode === k} onClick={() => setMode(k)}
              className={cx("rounded-[9px] border px-3.5 py-2 text-[13.5px]", mode === k ? "border-accent bg-soft font-semibold" : "border-line bg-white hover:border-mute-4")}>
              {label}
            </button>
          ))}
        </div>
        <div className="mt-5 flex items-center gap-3 border-t border-divider pt-4">
          <span className="flex-1 text-[13.5px] text-mute-2">Keyboard shortcuts make the everyday things one key away.</span>
      ...
      <Card className="mb-8 p-6">
        {standalone ? (
          <p className="m-0 text-[14px] text-mute-1">You are using the installed app. It updates itself whenever the team ships a change.</p>
        ) : (
          <div className="flex flex-wrap items-center gap-4">
            <p className="m-0 min-w-[240px] flex-1 text-[14px] text-mute-1">Install BrandOS on this {ios ? "iPhone or iPad" : "computer or phone"}. It opens in its own window, gets a home-screen icon, and loads faster.</p>
      ...
      {ws.can("access") && (
        <>
          ...
          <Card className="mb-8 flex flex-wrap items-center gap-4 p-6">
            <p className="m-0 min-w-[240px] flex-1 text-[14px] text-mute-1">Download everything — clients, brands, offers, assets, discussion and activity — as one JSON file. Uploaded files are not included.</p>
            <a href="/api/export" className="rounded-[9px] border border-line bg-white px-4 py-2 text-[13.5px] font-semibold text-ink-3 hover:border-mute-2">Export JSON</a>
          </Card>
        </>
      )}
