  ...
  return (
    <Modal title={draft?.id ? "Edit access" : "Invite someone"} sub="Two decisions: what they can do, and which clients they can see." width={560} onSubmit={save}
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="Name"><input className="field text-[13.5px]" value={d.name} onChange={(e) => setD({ ...d, name: e.target.value })} placeholder="Marta Vieira" /></Field>
          ...
          {ACCESS_LEVELS.map((a) => (
            <button key={a} type="button" role="radio" aria-checked={d.access === a} onClick={() => setD({ ...d, access: a })}
              ...
              <span className="flex-1">
                <span className="block text-[13.5px] font-semibold">{a}</span>
                <span className="mt-0.5 block text-[12px] leading-[1.45] text-mute-1">{ACCESS_NOTE[a]}</span>
              </span>
            </button>
          ))}
        </div>
      ...
      <div>
        ...
        <button type="button" role="checkbox" aria-checked={d.allClients} onClick={() => setD({ ...d, allClients: !d.allClients, clientIds: [], brandIds: [], groupIds: [] })}
          ...
          <Tick on={d.allClients} /><span className="text-[13.5px] font-medium">Every client, including ones added later</span>
        </button>
        <div style={{ opacity: d.allClients ? 0.4 : 1 }}>
          ...
          <div className="mb-3 flex flex-wrap gap-1.5">
            {ws.d.groups.map((g) => {
              ...
              return (
                <button key={g.id} type="button" aria-pressed={on} onClick={() => toggle("groupIds", g.id)} className={cx("rounded-full border px-[13px] py-1.5 text-[12.5px] font-medium transition", on ? "border-accent bg-soft" : "border-line bg-white")}>
                  {g.name} <span className="text-mute-3">· {g.clientIds.length} clients</span>
                </button>
              );
            })}
            {!ws.d.groups.length && <span className="text-[12px] text-mute-3">No groups yet.</span>}
          </div>
          ...
          <div className="mb-3 flex flex-wrap gap-1.5">
            {ws.d.clients.filter((c) => !c.archived).map((c) => {
              ...
              return <button key={c.id} type="button" aria-pressed={on} onClick={() => toggle("clientIds", c.id)} className={cx("rounded-full border px-[13px] py-1.5 text-[12.5px] font-medium transition", on ? "border-accent bg-soft" : "border-line bg-white")}>{c.name}</button>;
            })}
          </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {ws.d.brands.filter((b) => !b.archived).map((b) => {
              ...
              return (
                <button key={b.id} type="button" aria-pressed={on} onClick={() => toggle("brandIds", b.id)} className={cx("flex items-center gap-1.5 rounded-full border px-[13px] py-1.5 text-[12.5px] font-medium transition", on ? "border-accent bg-soft" : "border-line bg-white")}>
...
export function GroupModal({ draft }: { draft?: Partial<Group> }) {
  ...
  return (
    <Modal title={draft?.id ? "Edit group" : "New group"} sub="A saved set of clients. Everyone in the group sees all of them." width={540} onSubmit={save} footer={<Footer saveLabel="Save group" pending={pending} disabled={!d.name.trim()} />}>
      <Field label="Name"><input className="field text-[13.5px]" value={d.name} onChange={(e) => setD({ ...d, name: e.target.value })} placeholder="Nonprofit pod" /></Field>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {ws.d.clients.filter((c) => !c.archived).map((c) => {
            const on = d.clientIds.includes(c.id);
            return <button key={c.id} type="button" aria-pressed={on} onClick={() => toggle("clientIds", c.id)} className={cx("rounded-full border px-[13px] py-1.5 text-[12.5px] font-medium", on ? "border-accent bg-soft" : "border-line bg-white")}>{c.name}</button>;
          })}
        </div>
      </div>
      <div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {ws.d.users.filter((u) => !u.allClients).map((u) => {
            ...
            return (
              <button key={u.id} type="button" aria-pressed={on} onClick={() => toggle("memberIds", u.id)} className={cx("flex items-center gap-1.5 rounded-full border py-1 pl-1 pr-3 text-[12.5px] font-medium", on ? "border-accent bg-soft" : "border-line bg-white")}>
                <Avatar initials={u.initials} size={20} />{u.name}
              </button>
            );
          })}
        </div>
        <div className="mt-2 text-[11.5px] text-mute-3">People who already see every client are not listed.</div>
      </div>
    </Modal>
  );
}
    ...
    <Modal title="Who should review this?" sub={x?.name} width={440} footer={<Btn variant="ghost" onClick={close}>Cancel</Btn>} bodyClass="gap-0 px-3 pb-3.5 pt-2">
      {people.map((u) => (
        <button key={u.id} type="button" disabled={pending} onClick={async () => { const r = await run(sendForReview, item, id, u.id); if (r.ok) close(); }}
          ...
          <span className="flex-1">
            <span className="block text-[13.5px] font-semibold">{u.name}{u.id === ws.me.id ? " (you)" : ""}</span>
            <span className="block text-[12.5px] text-mute-2">{u.role} · {u.access}</span>
          </span>
          <span className="font-mono text-[11px] text-mute-4" title="Items already waiting on them">{ws.d.queueCounts[u.id] ?? 0} waiting</span>
        </button>
      ))}
      {!people.length && <div className="p-6 text-center text-[13px] text-mute-2">Nobody else can review yet. Give someone Reviewer access on the Team page.</div>}
    </Modal>
  );
}
