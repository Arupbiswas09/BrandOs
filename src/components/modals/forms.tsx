...
export function ClientModal({ draft }: { draft?: Partial<Client> }) {
  ...
  return (
    <Modal title={draft?.id ? "Edit client" : "New client"} width={480} onSubmit={save} footer={<Footer saveLabel="Save client" pending={pending} disabled={!d.name.trim()} />}>
      <Field label="Client name"><input className="field text-[13.5px]" value={d.name} onChange={(e) => setD({ ...d, name: e.target.value })} placeholder="Quokka For Good" /></Field>
...
function ColourInput({ value, onChange, label }: { value: string; onChange: (v: string) => void; label: string }) {
  ...
  return (
    <span className="flex items-center gap-2 rounded-[9px] border border-line bg-white py-1 pl-1 pr-2">
      ...
      <input value={text} aria-label={`${label} hex`} onChange={(e) => { setText(e.target.value); if (isHex(e.target.value)) onChange(e.target.value.toUpperCase()); }} className="w-[76px] border-0 bg-transparent font-mono text-[12px] outline-none" />
    </span>
  );
}
    ...
    <Modal title={draft.id ? "Edit brand" : "New brand"} sub={parent ? `A wing of ${parent.name}` : undefined} width={540} onSubmit={save} footer={<Footer saveLabel="Save brand" pending={pending} disabled={!d.name.trim()} />}>
      <div className="flex items-center gap-3.5 rounded-xl p-[18px]" style={{ background: hexA(d.primary, 0.06) }}>
        ...
        <span className="flex-1">
          ...
          <span className="mt-0.5 block text-[11.5px] text-mute-1">{draft.id ? d.tagline || "No tagline yet" : "Starts with five default goals you can edit or merge later"}</span>
        </span>
        <span className="h-6 w-6 rounded-md" style={{ background: d.secondary }} title="Secondary colour" />
      </div>
      <Field label="Brand name"><input className="field text-[13.5px]" value={d.name} onChange={(e) => setD({ ...d, name: e.target.value })} placeholder="Acme SaaS" /></Field>
        ...
        <div className="flex flex-col gap-2">
          {segments.map((s) => (
            <div key={s.key} className="flex items-center gap-2">
              ...
              <span className="w-[62px] flex-none text-right text-[11.5px] text-mute-3">{s.orig ? counts(s.orig) : 0} offers</span>
          ...
          <div className="flex items-center gap-2 text-[12px] text-mute-3"><Chip color="#6E7C76">All segments</Chip> is always there for offers that speak to everyone.</div>
...
export function ServiceModal({ draft }: { draft: Partial<Service> & { brandId: string } }) {
  ...
  return (
    <Modal title={draft.id ? "Edit service" : "New service"} sub={`In ${ws.brand(d.brandId)?.name}. A capability you sell — offers hang off it, one per segment.`} onSubmit={save} footer={<Footer saveLabel="Save service" pending={pending} disabled={!d.name.trim()} />}>
      ...
      <Field label="Service name"><input className="field text-[13.5px]" value={d.name} onChange={(e) => setD({ ...d, name: e.target.value })} placeholder="Google Ad Grant" /></Field>
  ...
  return (
    <Modal title={draft.id ? "Edit CTA" : "New CTA"} width={480} onSubmit={save} footer={<Footer saveLabel="Save CTA" pending={pending} disabled={!d.text.trim()} />}>
      <div className="flex items-center justify-center rounded-[11px] bg-wash-2 p-6">
        <span className="inline-block rounded-[7px] px-[18px] py-[9px] text-[13.5px] font-semibold" style={preview}>{d.text || "Button text"}</span>
      </div>
      <Field label="Button text"><input className="field text-[13.5px]" value={d.text} onChange={(e) => setD({ ...d, text: e.target.value })} placeholder="Book a Grant Audit" maxLength={80} /></Field>
...
export function GoalModal({ brandId, name, description, original }: { brandId: string; name?: string; description?: string; original?: string }) {
  ...
  return (
    <Modal title={original ? "Edit goal" : "New goal"} sub={`In ${ws.brand(brandId)?.name}. Keep it broad enough that a tactic never fits it exactly.`} width={480} onSubmit={save}
      footer={<Footer saveLabel="Save goal" pending={pending} disabled={!d.name.trim()} left={original ? <button type="button" onClick={remove} className="px-0.5 py-1.5 text-[12px] text-danger hover:underline">Remove from every offer</button> : undefined} />}>
      <Field label="Goal"><input className="field text-[13.5px]" value={d.name} onChange={(e) => setD({ ...d, name: e.target.value })} placeholder="Audience growth" /></Field>
  ...
  return (
    <Modal title={`Merge ${from} into…`} sub={`${n} ${n === 1 ? "offer moves" : "offers move"} across. ${from} then disappears.`} width={460} onSubmit={save} footer={<Footer saveLabel="Merge" pending={pending} disabled={!into} />} bodyClass="gap-[7px]">
      {(ws.brand(brandId)?.goals ?? []).filter((g) => g.name !== from).map((g) => (
        <button key={g.name} type="button" aria-pressed={into === g.name} onClick={() => setInto(g.name)} className={cx("w-full rounded-[10px] border px-3.5 py-[11px] text-left text-[13.5px] font-medium transition", into === g.name ? "border-accent bg-soft" : "border-line bg-white")}>{g.name}</button>
      ))}
    </Modal>
  );
}
      ...
      {list.map((o) => (
        <button key={o.id} type="button" onClick={() => { close(); router.push(href.offer(o.id)); }} className="flex w-full items-center gap-[11px] rounded-[10px] border border-line bg-white px-[13px] py-[11px] text-left hover:border-mute-2">
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-0.5 block text-[12px] text-mute-2">{o.serviceId ? ws.service(o.serviceId)?.name ?? "Standalone" : "Standalone"} · {o.segment}</span>
          </span>
          <Chip color={OFFER_STATUS[o.status]} size="xs">{o.status}</Chip>
        </button>
      ))}
      {!list.length && <div className="p-7 text-center text-[13px] text-mute-2">No offer chases this goal yet.</div>}
    </Modal>
  );
}
