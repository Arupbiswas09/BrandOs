...
export async function seed(db: DB) {
  ...
  const people: Array<Partial<s.User> & { id: string; name: string; initials: string; role: string }> = [
    ...
    { id: "jw", name: "Joss Weekes", initials: "JW", role: "Client lead", access: "Admin", allClients: true },
  ...
  const users = people.map((p) => ({
    ...
    email: `${p.name.split(" ")[0].toLowerCase()}@quokkaforgood.org`,
    ...p,
  }));
  ...
  const brands: s.Brand[] = [
    ...
    },
    {
      ...
      guidelines: [], goals: goals(), archived: false, createdAt: at("8 months ago"), updatedAt: at("5 days ago"),
    },
    {
  ...
  const C = (id: string, kind: "asset" | "offer", itemId: string, userId: string, text: string, when: string, extra: Partial<s.Comment> = {}): s.Comment =>
    ({ id, kind, itemId, userId, text, createdAt: at(when), resolved: false, isChange: false, refs: [], mentions: [], ...extra });
  const comments = [
